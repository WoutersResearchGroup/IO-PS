...
NumItersMOR = 18; % Number of iterations required for the Method of Reflections
NumItersF_C = 18;
...
GVCMapping = csvread('Auto Value Chain - updated for H5 - v2.csv'); %GVCData 1) Tier#; 2) GVC Activity#; 3) HS Code
...
CountryCA = AllMat((AllMat(:,2)== CountryNum),:);
CountryRaw = zeros(size(CountryCA,1),7);
CountryRaw(:,1) = CountryCA(:,1); 
CountryRaw(:,[3 4]) = CountryCA(:,[3 4]); 

[SARCAMat,ProductSum] = TransformCountry(AllMat,CountryRaw,Products); 

WorldRCA = CalculateAllRCA(AllMat);

%% Generate M matrixes

[M,MAbs,Countries] = generateM(Products,AllMat,WorldRCA);

dlmwrite('M.txt',M,'precision',10);
dlmwrite('MAbs.txt',MAbs,'precision',10);
dlmwrite('Countries.txt',Countries,'precision',10);
...
[CountryCompInd,ProductCompInd] = CalcComplexity(M,Products,Countries,NumItersMOR, NumItersF_C,ComplexityType);

dlmwrite('CountryCompInd.txt',CountryCompInd,'precision',10);
dlmwrite('ProductCompInd.txt',ProductCompInd,'precision',10);
...
dlmwrite('CP.txt',CP,'precision',10);
dlmwrite('Prox.txt',Prox,'precision',10);
...
dlmwrite('DistanceAndOpporGain.txt',DistanceAndOpporGain,'precision',10);
dlmwrite('Densities.txt',Densities,'precision',10);
dlmwrite('Distance.txt',Distance,'precision',10);
dlmwrite('ProxSums.txt',ProxSums,'precision',10);
...
[Comp2 CompPS2] = CalcCompContr2(GVCAct,NumProductsCurInGVC,CurrentSumOfComplexityInGVC,SumCompContr,NumberInActToBeAdded,CurrentSumOfComplexityInPS,NumProductsCurInPS); % Calculate Complexity values for Different Activities based on the unexploited items

dlmwrite('CompContr2.txt',CompPS2,'precision',10);
...
[BestFrom2Comp BestFrom2Oppor BestComplexityContr BestOpporGain] = CalcBestFrom2(CompPS2,DistReq2,OpporGainScenarios2,ScenariosDistances,BestFrom1Comp,BestFrom1Oppor,BestComplexityContr,BestOpporGain);
...
[Comp3 CompPS3] = CalcCompContr3(GVCAct,NumProductsCurInGVC,CurrentSumOfComplexityInGVC,SumCompContr,NumberInActToBeAdded,CurrentSumOfComplexityInPS,NumProductsCurInPS); % Calculate Complexity values for Different Activities based on the unexploited items

dlmwrite('CompContr3.txt',CompPS3(3,3,:),'precision',10);
...
[BestFrom3Comp BestFrom3Oppor BestComplexityContr BestOpporGain] = CalcBestFrom3(CompPS3,DistReq3,OpporGainScenarios3,ScenariosDistances,BestFrom2Comp,BestFrom2Oppor,BestComplexityContr,BestOpporGain);

BestFromHeadings = {'1) Activity1' ' 2) Activity 2' ' 3) Activity 3' ' 4) New average complexity' ' 5) OpporGain' ' 6) Distance used'};

csvwrite('BestFrom3Comp.csv',BestFrom3Comp)
csvwrite('BestFrom3Oppor.csv',BestFrom3Oppor)

% BestFrom3CompOut = [BestFromHeadings;num2cell(BestFrom3Comp)];
% BestFrom3OpporOut = [BestFromHeadings;num2cell(BestFrom3Oppor)];
% 
% writecell(BestFrom3CompOut,'BestFrom3Comp.xlsx');
% writecell(BestFrom3OpporOut,'BestFrom3Oppor.xlsx');
...
function [WorldRCA] = CalculateAllRCA(AllMat)



%This macro transforms the raw data to calculate RCA values

% The resulting Arrays are as follows:
WorldRCA = zeros(size(AllMat,1),1); % RCA for AllMat row
TotalCountryExport = zeros(max(AllMat(:,2)),1);  % Total Export of country
TotalProductExport = zeros(max(AllMat(:,3)),1);  % Total Export of country

%AllMat; % Format = ( 1 Year; 2 Country; 3 hs92code; 4 Export Value; 5 Import value: 6 Export RCA; 7 Import RCA)
%CountryRaw; % Format =  ( 1 Year; 2 Country; 3 hs92code; 4 Export Value; 5 Import value: 6 Export RCA; 7 Import RCA)

Progress = 'CalculateAllRCA_Start'
...
for j = 1:size(AllMat,1) %Run through all raw world Exports entries
    ...
    TotalCountryExport(Country,1) = TotalCountryExport(Country,1) + AllMat(j,4);
    TotalProductExport(Product,1) = TotalProductExport(Product,1) + AllMat(j,4);
    WorldExport = WorldExport + AllMat(j,4);
...
for j = 1:size(AllMat,1) %Run through all raw world Exports entries
    ...
    CountryPercentage = AllMat(j,4) ./ TotalCountryExport(Country,1);
    WorldPercentage = TotalProductExport(Product,1) ./ WorldExport;     
    WorldRCA(j,1) = CountryPercentage ./ WorldPercentage;
    
end

Progress = 'CalculateAllRCA_Finish'

end

function [M,MAbs,Countries] = generateM(Products,AllMat,WorldRCA)
...
CountriesDupl = AllMat(:,2); %Reads all country names into array (Sorted that names of countries are in alphabetical order)

Countries = unique(CountriesDupl); %Removes duplicates

ProductRCA = [AllMat(:,3) WorldRCA(:,1)]; %Filter only necessary parts of AllMat (Sorted that names of countries are in alphabetical order)
...
for k = 1:size(ProductRCA,1) %Run through all rows of All2014 array
    ...
    if k > 1
        if  CountriesDupl(k) == CountriesDupl(k-1)
        else
            c=c+1;
        end
    end
...
function [CountryCompInd,ProductCompInd] = CalcComplexity(M,Products,Countries,NumItersMOR, NumItersF_C,ComplexityType)


if ComplexityType == "MOR"

    Progress = 'CalcComplexity_Start_MOR'
    ...
    for n = 2:NumItersMOR  %Run through number of iterations as required
    ...
    Progress = 'CalcComplexity_Finish_MOR'


elseif ComplexityType == "ECI_Vector"
    
    Progress = 'CalcComplexity_Start_ECI_Vector'
    
 %Get Mc (= sum_p(Mcp)) ie kc0 ie diversity 
    ...
    Mcc = zeros(size(Countries,1),size(Countries,1)); %Indicates the number of countries that produce each product (array of ubiquity)
    ...
    for i = 1:size(Countries,1)
        ...
        for j = 1:size(Countries,1)
            ...
            for k = 1:size(Products,1)
                ...
                Mcc(i,j) = Mcc(i,j) + M(i,k) * M(j,k) / (Kp0(k) * Kc0(i));
    ...
    [Mcc_EigenVector,~] = eig(Mcc) ;
    
    ECI = Mcc_EigenVector(:,2); 
    ...
    for i = 1:size(Countries,1)
        ...
        CountryCompInd(i) = ( ECI(i) - mean(ECI(:)) ) / std(ECI(:));
    ...
    Mpp = zeros(size(Products,1),size(Products,1)); %Indicates the number of countries that produce each product (array of ubiquity)
    ...
    for i = 1:size(Products,1)
        ...
        for j = 1:size(Products,1)
            ...
            for k = 1:size(Countries,1)
                ...
                Mpp(i,j) = Mpp(i,j) + M(k,i) * M(k,j) / (Kp0(i) * Kc0(k));
    ...
    [Mpp_EigenVector,Mpp_EigenValues] = eig(Mpp) ;
    
    PCI = Mpp_EigenVector(:,2); 
    
    ProductCompInd = zeros(size(Products,1),1);
    
    for i = 1:size(Products,1)

        ProductCompInd(i) = ( PCI(i) - mean(PCI(:)) ) / std(PCI(:));

    end
    
    Progress = 'CalcComplexity_Finish_ECI_Vector'
    
    
    
    
elseif ComplexityType == "F_C"
    
    
    Progress = 'CalcComplexity_Start_F_C'
    
    
    F0 = ones(size(Countries,1),1); %Indicates the number of products each country produces (array of diversity)

    Q0 = zeros(size(Products,1),1); %Indicates the number of countries that produce each product (array of ubiquity)


 
    F1 = zeros(size(Countries,1),1); %Indicates the next iteration of complexity of each country (array of country complexity 1)
    ...
    for i = 1:size(Countries,1) %Run through countries
        ...
        for j = 1:size(Products,1) %Run through products
            ...
            ComplexitySum = ComplexitySum + M(i,j)*Q0(j);
    ...
    Q1 = zeros(size(Products,1),1); %Indicates the next iteration of complexity products (array of product complexity 1)
    ...
    for i = 1:size(Products,1) %Run through Products
        ...
        for j = 1:size(Countries,1) %Run through Countries
            ...
            ComplexitySum = ComplexitySum + ( M(j,i) * (1 / F0(j)) )  ;

        end
        
        Q1(i) = 1 / ComplexitySum ;
    ...
    F2 = zeros(size(Countries,1),1); %Indicates the next iteration of complexity of each country (array of country complexity 1)
    Q2 = zeros(size(Products,1),1); %Indicates the next iteration of complexity products (array of product complexity 1)


    for n = 2:NumItersF_C  %Run through number of iterations as required
        ...
        for i = 1:size(Countries,1) %Run through countries
            ...
            for j = 1:size(Products,1) %Run through products
                ...
                ComplexitySum = ComplexitySum + M(i,j)*Q1(j);
        ...
        for i = 1:size(Products,1) %Run through Products
            ...
            for j = 1:size(Countries,1) %Run through Countries
                ...
                ComplexitySum = ComplexitySum + ( M(j,i) * (1 / F1(j)) )  ;

            end

            Q2(i) = 1 / ComplexitySum ;
    ...
    for i = 1:size(Countries,1)
        ...
        CountryCompInd(i) =  FN(i) / mean(FN(:)) ;

    end

    for i = 1:size(Products,1)

        ProductCompInd(i) =  QN(i) / mean(QN(:))  ;

    end
    
    
    Progress = 'CalcComplexity_Finish_F_C'
    
else
    
    Progress = 'Invalid_Complexity_Type'
        ...
        if RCA(j,2) > 1 % If you are already competitive; calculate Density from competitive to opportunity otherwise Distance and open forest
            ...
            %if i ~= j % Make sure product is not not itself
                ...
                if RCA(i,2) < 1 % Check if original activity is indeed an opportunity
                    ...
                    OpenForestContrSum(i) = OpenForestContrSum(i) + Prox(i,j)*ProductCompInd(j)/(ProxSums(j)); 
...
%     CheckSumSA(i) = CheckSumSA(i) / (sum(Prox6D(i,:)) - 1);
    ...
    Densities(i) = DensityNumerator / (ProxSums(i));
    Distance(i) = DistanceNumerator / (ProxSums(i));
...
function [Comp2 CompPS2] = CalcCompContr2(GVCAct,NumProductsCurInGVC,CurrentSumOfComplexityInGVC,SumCompContr,NumberInActToBeAdded,CurrentSumOfComplexityInPS,NumProductsCurInPS)
...
CompPS2 = zeros(NumGVCAct,NumGVCAct,1); % 1) Complexity In PS for scenario (per activity)
...
for i = 1:NumGVCAct
    ...
    for j = 1:NumGVCAct
        ...
        if i ~= j
            ...
            CompPS2(i,j) = (TotalSumCompCont(i,j) + CurrentSumOfComplexityInPS) ./ (TotalNumberInActToBeAdded(i,j) + NumProductsCurInPS);
...
function [Comp3 CompPS3] = CalcCompContr3(GVCAct,NumProductsCurInGVC,CurrentSumOfComplexityInGVC,SumCompContr,NumberInActToBeAdded,CurrentSumOfComplexityInPS,NumProductsCurInPS)
        ...
        if i ~= j
            ...
            for k = 1:NumGVCAct
                ...
                if (k ~= j) &&  (k ~= i)
                    ...
                    CompPS3(i,j,k) = (TotalSumCompCont(i,j,k) + CurrentSumOfComplexityInPS) ./ (TotalNumberInActToBeAdded(i,j,k) + NumProductsCurInPS);
...
function [BestFrom1Comp BestFrom1Oppor BestComplexityContr BestOpporGain] = CalcBestFrom1(Round1Metrics,ScenariosDistances,BaseLineMetrics,CompInPS)
...
BestFrom3Comp = BestFrom2Comp; % 1) Activity1; 2) Activity 2; 3) Activity 3; 4) BestComplexity Contr; 5) OpporGain; 6) Distance
